...
(function () {
  ...
  let pages = [];
  function log(...args) {
    console.log("[standalone-mcp]", ...args);
  }
  ...
  function start() {
    ...
    ws.onopen = () => {
      ...
      const init = {
        ...
        params: {
          ...
          capabilities: { tools: {} },
          clientInfo: { name: "standalone-mcp-demo", version: "1.0.0" },
        },
      };
      send(init);
    };
    ws.onmessage = (ev) => {
      try {
        ...
        log("Failed to parse MCP message", err);
      }
    };
