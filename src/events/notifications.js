...
class NotificationManager {
  ...
  addChannel(name, config) {
    ...
    loggers.app.info(`Channel added: ${name} (${config.type})`);
  }
  ...
  removeChannel(name) {
    if (this.channels.has(name)) {
      ...
      loggers.app.info(`Channel removed: ${name}`);
      return true;
    }
    return false;
  }
  ...
  addTemplate(name, template) {
    ...
    loggers.app.info(`Template added: ${name}`);
  }
  ...
  removeTemplate(name) {
    if (this.templates.has(name)) {
      ...
      loggers.app.info(`Template removed: ${name}`);
      return true;
    }
    return false;
  }
  ...
  addRule(rule) {
    ...
    loggers.app.info(`Rule added: ${rule.id}`);
  }
  ...
  removeRule(ruleId) {
    ...
    if (index !== -1) {
      ...
      loggers.app.info(`Rule removed: ${ruleId}`);
      return true;
    }
    return false;
  }
  ...
  async send(notification, data = {}, context = {}) {
    try {
      ...
      loggers.app.info(`Notification sent: ${validatedNotification.title}`, {
      ...
      loggers.app.error('Notification send failed', { error: error.message, notification, data });
      throw error;
    }
  }
  ...
  async sendToChannels(notification, channels, context) {
    ...
    for (const channelName of channels) {
      ...
      try {
        ...
        loggers.app.error(`Channel error: ${channelName}`, {
          error: error.message,
  ...
  markAsRead(notificationId) {
    ...
    if (notification) {
      ...
      loggers.app.info(`Notification marked as read: ${notificationId}`);
      return true;
    }
    return false;
  }
  ...
  clearHistory(keep = 0) {
    ...
    loggers.app.info(`Notification history cleared, keeping ${keep} entries`);
  }
