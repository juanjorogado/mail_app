...
class StorageManager {
  ...
  initializeStorage() {
    try {
      ...
      loggers.storage.error('Error al inicializar almacenamiento', error);
      throw new Error('No se pudo inicializar el sistema de almacenamiento');
    }
  }

  /**
   * Cifra datos sensibles de cuenta (email y tokens)
   */
  ...
  /**
   * Descifra datos sensibles de cuenta
   */
  ...
  getEncryptionKey() {
    ...
    if (!secret) {
      ...
      // Solo para desarrollo, usar una clave por defecto
      console.warn('⚠️  Using default encryption key. Set APP_SECRET environment variable for production!');
      return crypto.scryptSync('default-secret-key-for-development-only', 'salt', 32);
    }
  ...
  /**
   * Guarda cuentas con datos sensibles encriptados
   */
  saveAccounts(accounts) {
    try {
      const encryptedAccounts = accounts.map(account => ({
        ...
        // Encriptar datos sensibles (email y tokens)
        sensitiveData: this.encryptSensitiveData(account)
      }));
  ...
  /**
   * Carga cuentas con datos sensibles desencriptados
   */
  loadAccounts() {
    try {
      if (!fs.existsSync(this.accountsFile)) {
      ...
      return encryptedAccounts.map(account => {
        // Compatibilidad hacia atrás: cuentas con formato antiguo
        if (account.tokens && !account.sensitiveData) {
          // Formato antiguo: tokens encriptados por separado
          const tokens = this.decrypt(account.tokens);
          return {
            id: account.id,
            alias: account.alias,
            provider: account.provider,
            email: account.email || '', // Email en texto plano en formato antiguo
            tokens: tokens,
            created: account.created,
            lastUpdated: account.lastUpdated
          };
        ...
        // Formato nuevo: datos sensibles encriptados juntos
        const sensitiveData = account.sensitiveData ?
          this.decryptSensitiveData(account.sensitiveData) : { email: '', tokens: null };
  ...
  /**
   * Migra cuentas del formato antiguo al nuevo formato encriptado
   */
  migrateAccountsToEncryptedFormat() {
    try {
      const accounts = this.loadAccounts();
      console.log(`Migrating ${accounts.length} accounts to encrypted format...`);

      // Re-guardar todas las cuentas (esto las encriptará con el nuevo formato)
      const migrated = this.saveAccounts(accounts);

      if (migrated) {
        console.log('✅ Migration completed successfully');
        return { success: true, migrated: accounts.length };
      } else {
